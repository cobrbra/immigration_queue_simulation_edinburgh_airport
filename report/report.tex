...
\begin{table}[!ht]
\caption{Historic datasets of Edinburgh Airport Arrivals.}
...
The competition data set does not contain info in the departing airport. As this is provided in the additional data set we can sample airport classifications for the competition data set conditional on the number of passengers on the flight. We assume that all flights in the competition data come from outside UK+ and hence their passengers need to pass through immigration.
...
The competition data set contains the number of passengers on board each flight. This information is not available for the additional historical data, but can be simulated with the aircraft type, its maximum capacity, and the load factor.

\paragraph{Coached or Contact}
The competition data gives an overall split of flights that are coached or contact for each year. As there was no additional information available we assume that the decision between coached or contact is independent of all flight characteristics. 


\subsubsection{Route}

\paragraph{Bulk Profile} 
When passengers are brought to the immigration hall by a bus they are arriving at the queue in rapid succession. We choose a XXX distribution  here.
...
\paragraph{Number of \glspl{egate} and desks} There are currently 10 \glspl{egate} and 10 staffed desks in use at Edinburgh Airport. As part of our simulations, these numbers will be increased.

\paragraph{\gls{egate} uptake} 
...
\paragraph{\gls{egate} failure rate}
A small percentage of passengers who use an \gls{egate} fail to pass through it successfully. This is set to kie between XX and XX.
