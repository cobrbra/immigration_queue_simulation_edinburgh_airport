...
Without changes to \gls{egate} eligibility and uptake, no amount of \gls{egate} construction will alleviate some pressure on Edinburgh Airport's desk border check queues. In Figure~\ref{fig:core_rec_fig} we see... \jbnote{not really sure what the message is here}.
...
\begin{figure}[!ht]
    \centering
    \figuretitle{XX}
    \includegraphics[width=\textwidth]{figures/robust1_placeholder.png}
     \caption{XX} \label{fig:robust_eligibility}
\end{figure}
...
%      \caption{\textbf{Left}: Core \gls{egate} construction recommendation and assumed progression of \gls{egate} eligibility and uptake. \textbf{Centre}: Wait time \glspl{kpi} averaged over XXX runs. \textbf{Right}: Queue length \glspl{kpi} averaged over XXX runs.} \label{fig:minus_core_rec_fig}
% \end{figure}
